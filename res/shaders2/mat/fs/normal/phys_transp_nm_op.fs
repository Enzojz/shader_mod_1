...
vec2 uvPos()
{	
	if (u_mat.opOpacity1 <= -2048.0) // Tangent-Bitangent
	{
...
void main() {
	...
	vec3 texNormal = decodeNormalScale(texture(normalTex, uv).rg, 1);
	vec3 normal = getNormalMapped(texNormal, normal_, binormal_, tangent_, u_mat.flipNormal);

	vec4 col = texture(albedoOpacityTex, uv);
	...
	if (isNonTrival())
	{
		col.rgb = applyOp(posAmbient.xyz, faceNormal, uv, opTex2, u_mat.opSettings2, u_mat.opScale2, u_mat.opOpacity2, col.rgb);
		fragmentOutputLightMap(posAmbient.xyz, ambient, normal, col.rgb * u_mat.albedoScales[matIndex].xyz, metalGlossAo.r, metalGlossAo.g, texCoordAlpha.z, alpha);
	} 
	else
	{
		if (u_mat.opOpacity1 > -128)
			col.rgb = applyOp(posAmbient.xyz, faceNormal, uv, opTex1, u_mat.opSettings1, u_mat.opScale1, u_mat.opOpacity1, col.rgb);
		col.rgb = applyOp(posAmbient.xyz, faceNormal, uv, opTex2, u_mat.opSettings2, u_mat.opScale2, u_mat.opOpacity2, col.rgb);
