...
vec2 uvPos(vec3 nrml, vec3 tangent, vec3 binormal, vec3 pos, vec3 texCoord, sampler2D tex, vec2 scale, float opacity)
{
	if (opacity == -256.0) 
	{
		...
		return vec2(
			dot(step(0.75, checkValueU), pos) + dot(1.0 - step(0.25, checkValueU), texCoord),
			dot(step(0.75, checkValueV), pos) + dot(1.0 - step(0.25, checkValueV), texCoord)
		) * scale;
	} else if (opacity == -512.0)
	{
		vec3 axis = normalize(texture(tex, vec2(0.5, 0.5)).xyz);
		vec3 xDir = normalize(cross(nrml, axis));
		vec3 yDir = cross(nrml, xDir);
		return vec2(dot(pos, xDir), dot(pos, yDir)) * scale;
	} else if (opacity == -1024.0)
	{
