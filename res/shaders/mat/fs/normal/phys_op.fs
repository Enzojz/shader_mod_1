...
vec2 uvPos()
{	
	if (opOpacity1 <= -1280.0) // Tangent-Bitangent
	{
...
void main() {
	...
	vec3 normal = getFaceNormal(normal_, flipNormal);
	...
	if (isNonTrival())
	{
		...
		fragmentOutputLightMap(posAmbient.xyz, ambient, normal, albedo * albedoScale, metalGlossAo.r, metalGlossAo.g, texCoordAlpha.z, 1.0);
	}
	else
	{
		if (opOpacity1 > -128)
